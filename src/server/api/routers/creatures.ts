...
export const creaturesRouter = createTRPCRouter({
  ...
  getByLocationId: publicProcedure
    .input(z.object({ locationId: CreatureSchema.shape.locationId }))
    .query(async ({ ctx, input }) => {
      return await ctx.prisma.creature.findMany({ where: { locationId: input.locationId } });
    }),
  ...
  edit: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const { id, ...data } = input;
      return await ctx.prisma.creature.update({ where: { id }, data });
    }),
  ...
  create: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.prisma.creature.create({ data: input });
    }),

  delete: publicProcedure
    .input(z.object({ id: CreatureSchema.shape.id }))
    .mutation(async ({ ctx, input: { id } }) => {
      return await ctx.prisma.creature.delete({ where: { id } });
    }),
});
